...
```{r}
# pbeta returns probability that value is greater then first parameter.
1-pbeta(.25,8,4)
1-pbeta(.5,8,4)
1-pbeta(.8,8,4)
```
 ...
 5) Posterior is Beta(8+33,4+40-33) = Beta(41,11)
    The first student gets 33 out of 40 questions right
```{r} 
...
# plot scaled likelihood for first student
lines(theta,44*dbinom(33,size=40,p=theta),lty=3)
legend("topleft", c("Posteriori", "Priorior", "MLE"))
```
...
```
 ...
 6) Second student This one has 24 questions right
...
```{r}
# rbeta random samples 1000 points from Beta(41, 11)
